...
TokenScript provides powerful directives for generating color systems programmatically.
...
baseColors
  /colorHarmony : complementary
    hue = 200          // Base hue (0-360)
    steps = 3          // Number of colors
    lightness = 50     // OKLCH lightness (0-100)
    chroma = 0.15      // OKLCH chroma (0-0.4)
```

**Supported harmony types:**
- `complementary` - Colors opposite on the color wheel (180°)
- `analogous` - Adjacent colors (±30° by default)
- `triadic` - Three evenly spaced colors (120° apart)
- `tetradic` - Four evenly spaced colors (90° apart)
- `balanced` - Evenly distributed colors around the wheel
- `monochromatic` - Variations of a single hue

**Parameters:**
- `hue` - Base hue angle (0-360), hex color, or OKLCH string
- `steps` - Number or array of step names
- `lightness` - OKLCH lightness value (0-100)
- `chroma` - OKLCH chroma value (typically 0-0.4)
- `distribution` - Custom angle distribution (for analogous/balanced)
...
Generate perceptual color scales with OKLCH interpolation:
...
blue
  /colorScale
    ...
    lightness = range(90, 20)    // Light to dark
    chroma = 0.15                // Constant chroma
```

**Parameters:**
- `hue` - Hue angle, hex color, or token reference
- `steps` - Number or array of step names
- `lightness` - Single value, `range(start, end)`, or `stops(val1, val2, ...)`
- `chroma` - Single value, `range(start, end)`, or `stops(val1, val2, ...)`
- `direction` - `normal` (default) or `inverted`

**Using range() and stops():**

```tokenscript
// Linear interpolation
lightness = range(90, 20)

// Custom stops with positions
lightness = stops(90:0%, 50:60%, 20:100%)

// With easing curves
lightness = stops(90, 20).interpolation(ease-in-out)
```

**Combining harmony and scales:**
...
scales
  /forEach [$baseColors] as color
    {color}
      /colorScale
        hue = $baseColors.{color}
        steps = [100, 200, 300, 400, 500]
        lightness = range(90, 30)
        chroma = 0.15
```

This creates a complete color system with three base colors, each with a 5-step scale.

## Next Steps

- Learn about [Typography Systems](./6.1-typography-systems.md)
